...
# --------------- supporting networks ----------------
def directional_attention_with_dense(rep_tensor, direction=None, scope=None,
                                     keep_prob=1., is_train=None, wd=0., activation='elu',
                                     tensor_dict=None, name=None):
    def scaled_tanh(x, scale=5.):
        return scale * tf.nn.tanh(1./scale * x)

    # rep_tensor = tf.transpose(rep_tensor, [1, 0, 2])
    ...
    with tf.variable_scope(scope or 'directional_attention_%s' % direction or 'diag'):
        ...
        rep_mask = tf.cast(tf.diag(- tf.ones([sl], tf.int32)) + 1, tf.bool)
        ...
        with tf.variable_scope('output'):
            ...
            output = tf.reshape(output,[10,64])
        # save attn
